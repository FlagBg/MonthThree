...
//echo $users['email']; //which is going to be exactly the same result;

//echo '<pre>' , var_dump( $users->fetch(PDO::FETCH_ASSOC )['email'] ), '</pre>' ;
//echo '<pre>' , var_dump( $users->fetch(PDO::FETCH_ASSOC)['email']), '</pre>' ;//this one prints exactly the key, that i want;
